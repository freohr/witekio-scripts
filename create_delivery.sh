...
# Create delivery Archive (CPU Only & ForLabVerification CPU Only)
echo "----------------"
echo $(pwd)
delivery_folder="/home/guest/Documents/NW/Deliveries/${app_version}_RTM"
mkdir -vp "${delivery_folder}" >> /dev/stdout

echo "----------------"
echo $(pwd)
archive_name="EGUI_SWUpdate_${app_version}_CPUOnly"
cd "${flash_folder}/EGUI\ ${app_version}/_SpecialPackages/${archive_name}"
7z a "${archive_name}.zip" -tzip -o"${delivery_folder}" *.bin >> /dev/stdout
cd -

echo "----------------"
archive_name="EGUI_SWUpdate_${app_version}_ForLabVerification_CPUOnly"
cd "${flash_folder}/EGUI\ ${app_version}/_SpecialPackages/${archive_name}"
7z a "${archive_name}.zip" -tzip -o"${delivery_folder}" *.bin >> /dev/stdout
cd -